...
$(function () {
	...
	var recentTmpl = '{{#each cachedAssets}}';
		...
		recentTmpl += '			<div class="span10"><i class="icon-refresh icon-spin load-seperator"></i><i class="icon-ok-circle"></i> <span><strong>{{attributes.overview_displayName}}</strong>正在被添加</span></div>';
		recentTmpl += '     </div>';
		recentTmpl += '{{/each}}';
